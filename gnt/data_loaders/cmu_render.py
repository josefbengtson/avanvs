...
from .cmu_data_utils import load_cmu_data, batch_parse_llff_poses

from scipy.io import savemat


class CMURenderDataset(Dataset):
    def __init__(self, args, scenes="fern", **kwargs):

        self.folder_path = os.path.join(args.rootdir, "data/cmu_render/")
        ...
        for i, scene in enumerate(scenes):
            ...
            _, poses, bds, render_poses, i_test, rgb_files = load_cmu_data(
            ...
            camera = 1
            if camera == 0:
                K = np.array([[868.993378, 0, 525.942323, 0], [0, 866.063001, 420.042529, 0],
                             [0, 0, 1, 0], [0, 0, 0, 1]])
            else:
                K = np.array([[873.382641, 0, 529.324138, 0], [0, 876.489513, 397.272397, 0],
                             [0, 0, 1, 0], [0, 0, 0, 1]])

            for idx in range(len(intrinsics)):
                intrinsics[idx, :, :] = K
            # dict_c2w = {"c2w": c2w_mats}
            #savemat("c2w_colmap_run08_c0_start_processed_10.mat", dict_c2w)
