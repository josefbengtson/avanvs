...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    poses_arr = np.load(os.path.join(basedir, "poses_bounds_run09_c1_end10_colmap.npy"))
    ...
    # if poses.shape[-1] != len(imgfiles) and  not skip:
...
#     hwf = c2w[:, 4:5]
#
#     # for theta in np.linspace(-np.pi * rots, np.pi * rots, N + 1)[:-1]:
#     # theta_start = np.pi/4
#     # theta_end = np.pi/3
#
#     theta_start = np.pi/3
...
#     z_max = 1
#
#     for dz in np.linspace(0, z_max, int(N / 2) + 2):
#
#         # render spiral 3
#         c = np.dot(
#             c2w[:3, :4],
#             np.array([np.cos(theta_start), -np.sin(theta_start), -np.sin(theta_start * zrate), 1.0]) * rads,
#         )
...
#
#     for theta in np.linspace(theta_start, theta_end, int(N/2) + 2)[1:]:
#         c = np.dot(
#             c2w[:3, :4],
...
#         new_pose = np.concatenate([viewmatrix(z, up, c), hwf], 1)
#         print("---------------------------------------------------------------------------")
#         print("new pose theta:")
#         print("dz: ", dz)
#         print("theta: ", theta)
#         # print("new render 1: \n", new_pose)
#         new_pose[2, 3] = new_pose[2, 3] + z_max
#         # print("new render: \n", new_pose)
#         print("---------------------------------------------------------------------------")
#
#         render_poses.append(new_pose)
#
#     for dz in np.linspace(z_max, 0, int(N / 2) + 2)[1:]:
#         # render spiral 3
#         c = np.dot(
#             c2w[:3, :4],
#             np.array([np.cos(theta_end), -np.sin(theta_end), -np.sin(theta_end * zrate), 1.0]) * rads,
#         )
#
#         z = normalize(c - np.dot(c2w[:3, :4], np.array([0, 0, -focal, 1.0])))
#         new_pose = np.concatenate([viewmatrix(z, up, c), hwf], 1)
...
#         render_poses.append(new_pose)
#
#     for theta in np.linspace(theta_end, theta_start, int(N/2) + 2)[1:]:
#         c = np.dot(
#             c2w[:3, :4],
#             np.array([np.cos(theta), -np.sin(theta), -np.sin(theta * zrate), 1.0]) * rads,
#         )
...
def render_path_spiral(c2w, up, rads, focal, zdelta, zrate, N):
    ...
    add_render_poses_theta(theta_start, theta_end, dz_start, num_poses_per_theta, render_poses, c2w, rads, focal, up, hwf)


    # add_render_poses_z(dz_start, dz_end, theta_start, num_poses_per_z, render_poses, c2w, rads, focal, up, hwf)
    # add_render_poses_theta(theta_start, theta_end, dz_end, num_poses_per_theta, render_poses, c2w, rads, focal, up, hwf)
    # add_render_poses_z(dz_end, dz_start, theta_end, num_poses_per_z, render_poses, c2w, rads, focal, up, hwf)
    # add_render_poses_theta(theta_end, theta_start, dz_start, num_poses_per_theta, render_poses, c2w, rads, focal, up, hwf)

    return render_poses


def add_render_poses_theta(theta_start, theta_end, dz, num_poses, render_poses, c2w, rads, focal, up, hwf):
