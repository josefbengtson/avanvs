...
def random_crop(rgb, camera, src_rgbs, src_cameras, size=(400, 600), center=None, rgb_cond2=None, src_rgbs_cond2=None):
    h, w = rgb.shape[:2]

    if rgb_cond2 is not None:
        h_cond2, w_cond2 = rgb_cond2.shape[:2]
        if h != h_cond2 or w != w_cond2:
            sys.exit("Images for condition 1 and 2 different sizes!")

    out_h, out_w = size[0], size[1]
    if out_w >= w or out_h >= h:
        if rgb_cond2 is None or src_rgbs_cond2 is None:
            return rgb, camera, src_rgbs, src_cameras
        else:
            return rgb, camera, src_rgbs, src_cameras, rgb_cond2, src_rgbs_cond2

    if center is not None:
        center_h, center_w = center
    else:
        # print("low: ", out_h // 2 + 1)
        # print("high: ", h - out_h // 2 - 1)
    ...
    if rgb_cond2 is not None and src_rgbs_cond2 is not None:
        rgb_out_cond2 = rgb_cond2[
                  center_h - out_h // 2: center_h + out_h // 2,
                  center_w - out_w // 2: center_w + out_w // 2,
                  :,
                  ]
        src_rgbs_cond2 = np.array(src_rgbs_cond2)
        src_rgbs_cond2 = src_rgbs_cond2[
                   :,
                   center_h - out_h // 2: center_h + out_h // 2,
                   center_w - out_w // 2: center_w + out_w // 2,
                   :,
                   ]
        return rgb_out, camera, src_rgbs, src_cameras, rgb_out_cond2, src_rgbs_cond2
    else:
