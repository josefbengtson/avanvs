...
class SpacesFreeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        scene_idx = 0
        all_views = self.all_views_scenes[scene_idx]
        num_rigs = len(all_views)
        print("Num rigs: ", num_rigs)
        selected_rig_id = np.random.randint(low=0, high=num_rigs)  # select a rig position
        rig_selected = all_views[selected_rig_id]
        print("TA")
        cam_id_selected = idx
        # cam_id_selected = np.random.choice(16)
        print("cam_id_selected: ", cam_id_selected)
        ...
        all_c2w_mats = self.all_c2w_scenes[scene_idx]
        print("b")

        all_rgb_paths = self.all_rgb_paths_scenes[scene_idx]
        all_intrinsics = self.all_intrinsics_scenes[scene_idx]
        all_img_sizes = self.all_img_sizes_scenes[scene_idx]
        ...
        far_depth = 100
        depth_range = torch.tensor([near_depth, far_depth])
        print("Spaces shape sizes")
        print("camera shape: ", render_camera.shape)
        print("src_rgbs shape: ", ref_rgbs_np.shape)
        print("src_cameras shape: ", np.stack(ref_cameras, axis=0).shape)
        # sys.exit("Spaces Shapes")
        return {
