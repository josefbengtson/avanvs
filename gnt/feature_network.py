...
class ClassifyLatent(nn.Module):
    def __init__(self, latent_size=16):
        super(ClassifyLatent, self).__init__()
        self.z_classifier = nn.Linear(latent_size, 4)
        # self.mean_pooling = nn.AvgPool2d(4, stride=4)
        # size_reduced = 5
        # feat_dim_in = 32
        # hidden_dim = 100
        # out_dim = 16
        # self.mean_pooling = nn.AdaptiveAvgPool2d((size_reduced, size_reduced))
        # self.z_fc = nn.Sequential(
        #     nn.Linear(size_reduced*size_reduced*feat_dim_in, hidden_dim),
        #     nn.ReLU(),
        #     nn.Linear(hidden_dim, out_dim),
        # )

    def forward(self,z):
        p = self.z_classifier(z)
        # print("Currently choosing just first featuremap!")
        # featmap = featmaps[0, :, :, :] # Choosing just first feature map
        # featmap_reduced = self.mean_pooling(featmap)
        # print("featmaps shape: ", featmap.shape)
        # print("featmaps reduced shape: ", featmap_reduced.shape)
        # sys.exit("ASD")
        # z = self.z_fc(torch.flatten(featmap_reduced))
        # print("z shape: ", z.shape)
        # print("p: ", p)
        # prediction = torch.argmax(p, dim=0)
